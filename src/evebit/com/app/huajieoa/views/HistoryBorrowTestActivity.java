...
import evebit.com.app.huajieoa.views.HistoryBorrowActivity.backButtonClickListener;
...
public class HistoryBorrowTestActivity extends Activity implements IXListViewListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		geneItems();
		mListView = (XListView) findViewById(R.id.HistoryTest_ListView);
		main = (String) getIntent().getExtras().get("main");// 接收传递数据
		MIDkey = (String)getIntent().getExtras().get("MIDkey");//接收子模块ID
	//	listView = (ListView) findViewById(R.id.History_ListView);
		backButton = (Button) findViewById(R.id.HistoryTest_Button_back);
		backButton.setOnClickListener(new backButtonClickListener());	
		searchImageView = (ImageView) findViewById(R.id.HistoryTest_imageButton_search);
		searchEditText = (EditText) findViewById(R.id.HistoryTest_search_EditText);
		tabberTextView = (TextView)findViewById(R.id.HistoryTest_tabBarText);
		historyButton = (Button)findViewById(R.id.HistoryTest_history);
		historyButton.setText("待办事务");
		...
		if (normal.note_Intent()) {// 判断是否有网络连接
			progressDialog = ProgressDialog.show(HistoryBorrowTestActivity.this, "", getResources().getString(R.string.get_date), true, false);
			thread();
	...
	//	mListView.setAdapter(mAdapter);
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				Log.v("--------", arg2 + "");
				Intent intent;
				intent = new Intent(HistoryBorrowTestActivity.this,
				...
				intent.putExtra("MName", mNameList.get(arg2-1));
	...
	class historyButtonOnClick implements OnClickListener{
		...
		public void onClick(View v) {
			// TODO Auto-generated method stub
			Intent intent;
			intent = new Intent(HistoryBorrowTestActivity.this,FinanciaBorrowActivity.class);
			intent.putExtra("main",main);
			intent.putExtra("MIDkey",MIDkey);
			startActivity(intent);
			finish();
	...
	private Handler handler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			if (blockdata.size()>0) {
				...
				mAdapter = new SimpleAdapter(HistoryBorrowTestActivity.this,
			...
			mListView.setAdapter(mAdapter);
			mListView.setXListViewListener(HistoryBorrowTestActivity.this);
			}
			else {
	...
	public void onRefresh() {
		mHandler.postDelayed(new Runnable() {
			...
			public void run() {
				...
				page =0 ;
				mData.clear();
				Normal normal = new Normal(HistoryBorrowTestActivity.this);// 判断是否有网络连接
				if (normal.note_Intent()) {// 判断是否有网络连接
					thread();
				} else {
					Toast.makeText(getApplicationContext(), "请连接网络",
		...
		public void onClick(View v) {
			...
			if (searchEditText.getText().toString().equals("")) {
				AlertDialog.Builder builder = new AlertDialog.Builder(
						HistoryBorrowTestActivity.this);
				...
				progressDialog = ProgressDialog.show(HistoryBorrowTestActivity.this,"", "正在搜索，请稍后", true, false);
				xidStrings.clear();
				titleStrings.clear();
				mNameList.clear();
	...
	class backButtonClickListener implements OnClickListener
	{
		...
		public void onClick(View v) {
			// TODO Auto-generated method stub
			Intent intent = new Intent(HistoryBorrowTestActivity.this,
					MainActivity.class);
			intent.putExtra("main", main);
			startActivity(intent);
			finish();
